    ...
    int timeout_ms, int use_tls, MqttTlsCb cb)
{
    ...
    if (use_tls) {
        ...
        //start_measure("TLSHandshake");
        rc = wolfSSL_connect(client->tls.ssl); //tu jest wywoływany CLIENT HELLO - to jest cały handshake
        //end_measure(); //TLSHandshake
        if (rc != WOLFSSL_SUCCESS) { //connect state: FINISHED_DONE
            rc = MQTT_CODE_ERROR_TLS_CONNECT;
