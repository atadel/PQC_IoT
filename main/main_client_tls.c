...
#define TLS_HANDSHAKE_ITERATIONS 30
#define NO_PSK
...
//TLS CLIENT CERTS 65
//44
static const char* pqc_client_cert = 
"-----BEGIN CERTIFICATE-----\n"
"client_certificate_data\n"
"-----END CERTIFICATE-----";

static const char* pqc_client_key = 
"-----BEGIN PRIVATE KEY-----\n"
...
void obtain_time(void) {
    ...
    static int mqtt_tls_cb(MqttClient* client)
    {
        ...
        * an insecure version. */
        ...
        rc = wolfSSL_CTX_use_certificate_buffer(client->tls.ctx,
        ...
        #if !defined(NO_CERT)
            rc = wolfSSL_CTX_use_PrivateKey_buffer(client->tls.ctx, (unsigned char*)pqc_client_key, strlen(pqc_client_key), WOLFSSL_FILETYPE_PEM);
        
            if (rc != WOLFSSL_SUCCESS) {
                printf("[ERROR] Failed to load client private key: %d\n", rc);
                return rc;
            }
            //printf("[TLS] Client private key loaded successfully\n");
            //client cert
            rc= wolfSSL_CTX_use_certificate_buffer(client->tls.ctx, (unsigned char*)pqc_client_cert, strlen(pqc_client_cert), WOLFSSL_FILETYPE_PEM);
            if (rc != WOLFSSL_SUCCESS) {
                printf("[ERROR] Failed to load client certificate: %d\n", rc);
                return rc;
            }
           // printf("[TLS] Client certificate loaded successfully\n");
            ...
            */
                        ...
                        //printf("Po wolfSSL_new: %u\n", heap_caps_get_largest_free_block(MALLOC_CAP_8BIT));
                        if (client->tls.ssl == NULL) {
                            printf("[ERROR] wolfSSL_new failed\n");
                            return WOLFSSL_FAILURE;
                        }

                        rc = wolfSSL_UseKeyShare(client->tls.ssl, TLS_GROUP);
                        if (rc != WOLFSSL_SUCCESS) {
...
static void mqtt_tls_handshake_once_task(void* pvParameters)
{
    ...
    net.context = &mSockFd;
        //printf("[TLS LOOP] Heap before connect: free=%lu largest=%lu\n",
